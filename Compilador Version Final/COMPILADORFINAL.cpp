...
PUNTOYCOMA, COMA, ASIGNACION, SUMA, RESTA, FDT, ERRORLEXICO,NULO
} TOKEN;
...
void chequear(char* s) /* verifica si se encuentra el s en ts = false -> lo almacena y genera en MV*/
{
    ...
    {
        colocar(s);
        generar((char *)"declara",s,(char *)"Entera",(char *)"");
    }
}
...
void terminar(void)
{
    generar((char *)"Finalizar",(char *)"",(char *)"",(char *)"");
}

void asignar(reg_expresion iz,reg_expresion der)
{
    generar((char *)"almacena",extraer(&der),iz.nombre,(char *)"");
}

void leer(reg_expresion in)
{
    generar((char *)"leer",extraer(&in),(char *)"Entera",(char *)"");
}

void escribir(reg_expresion out)
{
    generar((char *)"escribir",extraer(&out),(char *)"Entera",(char *)"");
}
